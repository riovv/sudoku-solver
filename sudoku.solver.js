(function (global) {
    /*
        Create an unique namespace for gaijin sudoku solver.
    ...
    */   
    ...
    sudoku.solver = {
        ...
        */
        assign: function (values, square, digit) {
            var i,
                otherDigits = values[square].replace(digit, ""),
                nOtherDigits = otherDigits.length;
                               
            for (i = 0; i < nOtherDigits; i++) {
                values = sudoku.solver.eliminate(values, square, otherDigits[i]);
                if (!values) {
                    return false;
                }
            }
        ...
        eliminate: function (values, square, digit) {
            var i, result;               
            ...
            */
                ...
                result = true;
                for (ps in gajin.sudoku.PEERS) {
                    result = sudoku.solver.eliminate(values, sudoku..PEERS[ps], values[square]);
                    if (!result) {
                        return false;
                    }
                }
            ...
            /*
             If a unit is reduced to only one empty square, then
             put the digit that is left there.
            */
            for (unit in sudoku..UNITS[square]) {
    ...
    sudoku.solve = function () {
        ...
        var digit,
            values = sudoku.clone(sudoku..VALUES),
            gridValues = sudoku.parsePuzzle();
       
        // Try to assign values from the grid.
        for (square in gridValues) {
            if (values.hasOwnProperty(square)) {
                digit = gridValues[square];
                if (!sudoku.solver.assign(values, square, digit)) {
                    return false
                }
            }
        }
    ...
    sudoku.parsePuzzle = function () {
        var i, value,
            values = {},
            nodeList = global.document.querySelectorAll('table#gaijin-sudoku tr td input'),
            nNodes = nodeList.length;
           
        for (i = 0; i < nNodes; i++) {
            value = +nodeList[i].value;
            if (value > 0 && value < 10) {
                values[nodeList[i].id] = value;
            }
        }   
        return values;
    };
    ...
    global.gaijin = gaijin;
    return global.gaijin;
